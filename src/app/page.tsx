...
import { getDashboardData } from "@/lib/pricing/getDashboardData";

export const dynamic = "force-dynamic";

const HomePage = async () => {
  try {
    const { products, summary } = await getDashboardData();
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <Link
            href="/products/new"
            className="inline-flex rounded-md bg-[#0080FF] px-4 py-2 text-sm font-medium text-white hover:bg-[#0066cc]"
          >
            Add product
          </Link>
        </div>
